...
select * from tbl_book;
    /* select * from tbl_book where book_PublisherName = 'Scholastic'; */
...
/* drop table tbl_publisher;
   drop database db_LibraryManagement;  */
	...
	INSERT INTO tbl_book_loans
		...
		update tbl_book_loans set book_loans_DueDate = '3/12/22' where book_loans_BookID in (4,11)
...
CREATE PROC Query_1
AS
SELECT library.library_branch_BranchID AS [Branch ID], library.library_branch_BranchName AS [Branch Name], copies.book_copies_No_Of_Copies AS [Number of copies], book.book_title AS [Book Title] 
From tbl_book as book 
INNER JOIN tbl_book_copies AS copies ON book.book_BookID = copies.book_copies_BookID
INNER JOIN tbl_library_branch AS library ON library.library_branch_BranchID = copies.book_copies_BranchID
where book.book_Title = 'The Lost Tribe' AND library.library_branch_BranchName = 'Sharpstown'
GO
EXEC Query_1
...
CREATE PROC Query_2
AS
SELECT copies.book_copies_BranchID AS [Branch ID], library.library_branch_BranchName AS [Branch Name], book.book_Title AS [Book Title], copies.book_copies_No_Of_Copies AS [Number of Copies] FROM tbl_book AS book
INNER JOIN tbl_library_branch AS library ON library.library_branch_BranchID = copies.book_copies_BranchID
INNER JOIN tbl_book_copies AS copies ON book.book_BookID = copies.book_copies_BookID
...
select * from tbl_book_loans 
where book_loans_DueDate = '2/7/18'

CREATE PROC Query_4
AS
SELECT lab.library_branch_BranchName AS [Branch Name], book.book_Title AS [Book Title], bw.borrower_BorrowerName AS [Borrower Name], bw.borrower_BorrowerAddress AS [Borrower Address], loans.book_loans_DueDate AS [Due Date] FROM tbl_book AS book
INNER JOIN tbl_book_loans AS loans ON loans.book_loans_BookID = book.book_BookID
INNER JOIN tbl_borrower AS bw ON bw.borrower_CardNo = loans.book_loans_CardNo
INNER JOIN tbl_library_branch AS lab ON lab.library_branch_BranchID = loans.book_loans_BranchID
/*WHERE loans.book_loans_DueDate = '2/2/18' and lab.library_branch_BranchName = 'Sharpstown' */
WHERE loans.book_loans_DueDate = format (GETDATE(), 'd/MM/yy') and lab.library_branch_BranchName = 'Sharpstown'
GO
EXEC Query_4

/*
select GETDATE() 
select format (GETDATE(), 'd/MM/yy') as date 
select * from tbl_book_loans 
where book_loans_DueDate = format (GETDATE(), 'd/MM/yy') */
...
CREATE PROC Query_5
AS
SELECT branch.library_branch_BranchName AS [Branch Name], COUNT(loans.book_loans_BranchID) AS [Total Books Loaned] FROM tbl_book_loans AS loans
INNER JOIN tbl_library_branch AS branch ON branch.library_branch_BranchID = loans.book_loans_BranchID
...
(@bookcheckout INT = 5)
AS
SELECT bw.borrower_BorrowerName AS [Borrower Name], bw.borrower_BorrowerAddress AS [Borrower Address], COUNT(loans.book_loans_BookID) AS [Total Books Checked Out] FROM tbl_book_loans AS loans
INNER JOIN tbl_borrower AS bw ON loans.book_loans_CardNo = bw.borrower_CardNo
GROUP BY bw.borrower_BorrowerName, bw.borrower_BorrowerAddress
HAVING COUNT(bw.borrower_BorrowerName) >= @bookcheckout
GO
EXEC Query_6
...
CREATE PROC Query_7
AS
SELECT author.book_authors_AuthorName AS [Author Name], book.book_Title AS [Book Title], copies.book_copies_No_Of_Copies AS [No Of Copies] , branch.library_branch_BranchName AS [Branch Name] FROM tbl_book_authors AS author
INNER JOIN tbl_book AS book ON book.book_BookID = author.book_authors_BookID
INNER JOIN tbl_book_copies AS copies ON copies.book_copies_BookID = book.book_BookID
INNER JOIN tbl_library_branch AS branch ON branch.library_branch_BranchID = copies.book_copies_BranchID
